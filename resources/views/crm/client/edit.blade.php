        ...
        <div class="row">
            ...
            <div class="col-md-12 mb-4 mt-1">
                <div class="d-flex flex-wrap justify-content-between align-items-center">
                    <h4 class="font-weight-bold">Редактирование клиента
                        <div class="badge badge-pill border border-dark text-dark">ID {{ $client->id }}</div>

                        <a class="ml-1" href="{{ route('crm.client.spam', $client->id) }}">
                            <div class="badge badge-pill border border-{{ $client->spam ? "success" : "danger"}} text-{{ $client->spam ? "success" : "danger"}}">
                                {{ $client->spam ? "Убрать из спама" : "Добавить в спам" }}
                            </div>
                        </a>
            ...
            <div class="col-md-8">
                <div class="card">
                    <div class="card-header d-flex justify-content-between">
                        ...
                        <a href="{{ route('crm.client.show', $client->id) }}" class="btn btn-outline-primary sml-btn but-square p-2">
                    ...
                    <div class="card-body">
                        ...
                        <form method="POST" action="{{route('crm.client.update', $client->id)}}">
                            @csrf
                            @method('PATCH')
                            ...
                            <div class="form-row mt-3">
                                ...
                                <input type="text" class="form-control" name="client[last_name]" value="{{ old('client[last_name]', $client->last_name) }}">
                                </div>
                                <div class="col">
                                <label for="first_name">Имя</label>
                                <input type="text" class="form-control" name="client[first_name]" value="{{ old('client[first_name]', $client->first_name) }}">
                                </div>
                            </div>
                            <div class="form-row mt-3">
                                <div class="col">
                                <label for="middle_name">Отчество</label>
                                <input type="text" class="form-control" name="client[middle_name]" value="{{ old('client[middle_name]', $client->middle_name) }}">
                                </div>
                                <div class="col">
                                ...
                                <select name="client[city_id]" class="form-control choicesjs">
                                    ...
                                    @foreach ($cities as $city)
                                        <option value="{{ $city->id }}" {{ $city->id === (int) old('client[city_id]', $client->city_id) ? 'selected' : '' }}>{{ $city->name_ru }}</option>
                                    @endforeach
                                </select>
                                </div>
                            </div>
                            ...
                            <div class="form-row mt-3">
                                <div class="col">
                                    ...
                                    <input type="tel" class="form-control" name="client[phone]" value="{{ old('client[phone]', $client->phone) }}">
                                </div>
                                <div class="col">
                                    <label for="additional_phone">Дополнительный телефон</label>
                                    <input type="tel" class="form-control" name="client[additional_phone]" value="{{ old('client[additional_phone]', $client->additional_phone) }}">
                                </div>
                            </div>
                            ...
                            <div class="mt-3">
                                ...
                                <textarea class="form-control" name="client[comment]" rows="3">{{ old('client[comment]', $client->comment) }}</textarea>
                                </div>
                            </div>
